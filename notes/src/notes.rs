fn main() {
    ...
    // We are NOT passing ownership of `s1`.
    // We are passing a REFERENCE to `s1`.
    // The `calculate_length` function is BORROWING `s1`.
    let len = calculate_length(&s1);

    // Because we only borrowed `s1`, we still own it.
    // This line is now perfectly valid!
    println!("The length of '{}' is {}.", s1, len);

    // Note: the original variable must be `mut` to allow for a mutable borrow.
    let mut s = String::from("hello");

    // Pass a mutable reference to `s`.
    change_string(&mut s);

    // We can still use `s` here because we only borrowed it.
    println!("{}", s); // Prints "hello, world"
}

// The type of `s` is `&String`, which reads "a reference to a String".
// This is also called a "string slice" if it's `&str`.
fn calculate_length(s: &String) -> usize {
    // `s` is a reference to the String. We can read its data.
    s.len()
} // Here, `s` goes out of scope. But because it does not own the data,
// `drop` is NOT called. The reference is simply gone.
// The original `s1` in `main` is unaffected.
